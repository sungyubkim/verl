...
def compute_offpolicy_metrics(
    ...
    training_ppl = verl_F.safe_exp(-mean_log_prob_training).mean()  # Batch mean of per-sequence PPL
    ...
    if rollout_log_prob is not None:
        ...
        k3_kl_matrix = verl_F.safe_exp(log_ratio) - log_ratio - 1
        ...
        rollout_ppl = verl_F.safe_exp(-mean_log_prob_rollout).mean()  # Batch mean of per-sequence PPL
        ...
        ppl_ratio = verl_F.safe_exp(log_ppl_diff).mean()  # mean(exp(log_ppl_diff)) = mean(ppl_ratio_i)
